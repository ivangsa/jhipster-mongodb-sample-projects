...
export class CustomerOrderUpdateComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.paymentDetailsService.query({ filter: 'customerorder-is-null' }).subscribe(
            (res: HttpResponse<IPaymentDetails[]>) => {
                if (!this.customerOrder.paymentDetails || !this.customerOrder.paymentDetails.id) {
                    this.paymentdetails = res.body;
                } else {
                    this.paymentDetailsService.find(this.customerOrder.paymentDetails.id).subscribe(
                        (subRes: HttpResponse<IPaymentDetails>) => {
                            this.paymentdetails = [subRes.body].concat(res.body);
                        },
